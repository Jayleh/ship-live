function getAwaiting() {
  d3.json("/awaiting", (error, ordersData) => {
    ...
    let paginationId = "#awaiting-pagination",
      pageLimit = 11;
...
function getOnHold() {
  d3.json("/on-hold", (error, ordersData) => {
    ...
    let paginationId = "#on-hold-pagination",
      pageLimit = 4;
...
function fillTable(ordersData, tableId) {
  ...
  for (let i = 0, ii = ordersData.orders.length; i < ii; i++) {
    ...
    $tbody
      ...
      .html(
        `<td class='text-left' style='color: ${getColor(
          age[0],
          age[1]
        )}'><strong>${age[0]}d ${
        age[1]
        }h</strong></td><td class='text-left'>${orderNum}</td><td class='text-left'>${recipient}</td><td>${itemName}</td><td>${numProducts}</td><td>${weekday}, ${month}/${date}</td>`
      );
  }
...
// Pagination function
function paginate(tableId, paginationId, pageLimit) {
  ...
  $(function () {
    ...
    function showPage(whichPage) {
      ...
      $(`${paginationId} li, #previous-page`).toggleClass("disabled", currentPage === 1);
      $(`${paginationId} li, #next-page`).toggleClass("disabled", currentPage === totalPages);
      return true;
    }
    ...
    $(document).on(
      "click",
      `${paginationId} li.current-page:not(.active)`,
      function () {
        return showPage(+$(this).text());
      }
    );

    $(`${paginationId} li, #next-page`).on("click", function () {
      return showPage(currentPage + 1);
    });

    $(`${paginationId} li, #previous-page`).on("click", function () {
      return showPage(currentPage - 1);
    });
  });
}
